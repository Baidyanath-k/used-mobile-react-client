...
const SymProduct = ({ symphonyProduct }) => {
    const { _id, title, img, orginal_price, resale_price } = symphonyProduct
    return (
        <div className="card card-compact w-72 bg-base-100 shadow-xl border-solid">
            ...
            <div className="card-body">
                ...
                <div className="card-actions justify-end">
                    <Link to={`/symphonyProduct/${_id}`}><button className="btn">Details</button></Link>
                </div>
            </div>
        </div>
    );
